...
public class LookupTable {
    ...
    public LookupTable(){
        ...
        "<inputNumber'> → <arithmetic> <inputNumber>", 
        "<inputNumber'> → EPSILON",
        "<declaration> → <datatype> <declaration'>", 
        "<declaration'> → <variableDeclaration>",
        "<declaration'> → <functionDeclaration>", 
        "<variableDeclaration> → VAR <varname> <variableDeclaration'>",
        "<variableDeclaration'> → = <value> <variableDeclaration'>",
        "<variableDeclaration'> → , <varname> <variableDeclaration'>",
        "<variableDeclaration'> → ;", 
        "<functionDeclaration> → FUNC <varname> ( <functionParameters> ) { <statement> }",
        "<functionParameters> → <datatype> <varname> <functionParameters'>",
        "<functionParameters'> → , <functionParameters>",
        "<functionParameters'> → EPSILON", 
        "<MainFunction> → MAIN { <statement> }",
        "<statement> → <datatype> <variableDeclaration> ; <statement>",
        "<statement> → <ifStatement> <statement>", 
        "<statement> → <whileStatement> <statement>", 
        "<statement> → <functionCall> ; <statement>",
        "<statement> → EPSILON", 
        "<functionCall> → FUNC <varname> ( <parameter> )",
        "<parameter> → <varname> <parameter'>", 
        "<parameter> → <value> <parameter'>",
        "<parameter'> → , <parameter>", 
        "<parameter'> → EPSILON", 
        "<ifStatement> → IF ( <condition> ) THEN { <statement> } <ifStatement'>",
        "<ifStatement'> → ELSE <elseStatement>", 
        "<ifStatement'> → EPSILON",
        "<elseStatement> → { <statement> } <ifStatement'>", 
        "<elseStatement> → <ifStatement>",
        "<whileStatement> → WHILE ( <condition> ) { <statement> }",
        "<condition> → <condition'> <conditionLogical>",
        "<conditionLogical> → <logical> <condition'> <conditionLogical>",
        "<conditionLogical> → EPSILON", 
